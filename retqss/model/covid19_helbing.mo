...
parameter Integer
	...
	GRID_SCENARIO = getIntegerModelParameter("GRID_SCENARIO", 0), //0=open (homogeneous), 1=school (heterogeneous)
	FORCE_TERMINATION_AT = getRealModelParameter("FORCE_TERMINATION_AT", 5);

// Movements and velocity parameters
parameter Real
	SPEED_MU = getRealModelParameter("SPEED_MU", 1.34),			// https://arxiv.org/pdf/cond-mat/9805244
	SPEED_SIGMA = getRealModelParameter("SPEED_SIGMA", 0.26),	// https://arxiv.org/pdf/cond-mat/9805244	    
...
// local variables
discrete Real _, normalX, normalY, ux, uy, volumeID, hx, hy, hz;
...
initial algorithm
    ...
    for i in 1:VOLUMES_COUNT loop
		...
		_ := volume_setProperty(i, "particleSpeed", random_normal(SPEED_MU, SPEED_SIGMA));
	...
	// Initial desired position is the same as the initial position
	for i in 1:N loop
		if GRID_SCENARIO == 0 then
...
algorithm
	...
	//EVENT: Next motivation update time: updates the particles acceleration
	when time > nextMotivationTick then
		nextMotivationTick := time + ACCELERATION_UPDATE_DT;
		for i in 1:N loop
			hx := dx[i];
			hy := dy[i];
			hz := dz[i];
		...
		_ := debug(INFO(), time, "Updating particles acceleration",_,_,_,_);
	end when;
	...
	when time > nextOutputTick then
		_ := covid19_outputCSV(time, N, x, y, VOLUMES_COUNT, volumeConcentration, recoveredCount, infectionsCount);
