...
PRT_setMetisOptions(idx_t options[METIS_NOPTIONS], QSS_data data)
{
  ...
  for(i = 0; i < t; i++)
  {
    options[PRT_getMetisOption(data->params->partitionerOptions.metis[i].lvalue)] =
        PRT_getMetisOption(
            data->params->partitionerOptions.metis[i].value);
  }
}
...
PRT_setScotchOptions(SCOTCH_Strat *strat, QSS_data data)
{
  ...
  for(i = 0; i < t; i++)
  {
    SCOTCH_stratGraphMapBuild(strat,
        PRT_getScotchOption(data->params->partitionerOptions.scotch[i].lvalue),
        data->params->lps,
...
PRT_generateIntStructure(int *n, int *e, int *np, int **x, int **xa, int **vw,
    int **ew, int **val, grp_t nvtxs, grp_t edges, grp_t nparts, grp_t *xadj,
    grp_t *adjcny, grp_t *vwgt, grp_t *ewgt, SD_PartitionMethod pm)
{
  int i;
  n[0] = nvtxs;
  e[0] = edges;
    ...
    switch(pm)
    {
      case SD_Metis:
        ...
        METIS_PartGraphKway(&nvtxs, &ncon, xadj, adjncy, vwgt, NULL, ewgt,
            &nparts, NULL, NULL, options, &edgecut, partition->values);
      }
        break;
      case SD_HMetis:
        {
        ...
        else
        {
          ...
          execlp("./khmetis", "./khmetis", hgraphName, parts, "5", "10", "1",
              "1", "0", "0", NULL);
          abort();
        }
      }
      ...
      case SD_Scotch:
        ...
        PRT_generateIntStructure(&n, &e, &np, &x, &xa, &vw, &ew, &val, nvtxs,
