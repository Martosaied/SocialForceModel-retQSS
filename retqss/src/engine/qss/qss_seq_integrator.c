...
void
QSS_SEQ_integrate(SIM_simulator simulate)
{
  ...
  while(t < ft)
  {
    ...
    switch(type)
    {
      case ST_State:
        ...
        if(QSS_stateLog_isFlagEnabled(simulator))
        {
            QSS_stateLog_log_x(index, simulator);
            for(i = 0; i < nSD; i++)
                QSS_stateLog_logState(SD[index][i], simulator);
        }
#endif
        if(nOutputs)
        {
...
#ifdef DEBUG
        if(QSS_stateLog_isFlagEnabled(simulator))
            QSS_stateLog_logState(j, simulator);
#endif
      }
        break;
      case ST_Event:
        ...
        if(event[index].zcSign != s || xOrder == 1 || et == t)
        {
          if(event[index].direction == 0 || event[index].direction == s)
          {
            ...
            for(i = 0; i < nLHSSt; i++)
            {
              ...
              if(QSS_stateLog_isFlagEnabled(simulator))
              {
                  QSS_stateLog_log_x(j, simulator);
                  for (h = 0; h < nSD; h++)
                      QSS_stateLog_logState(SD[j][h], simulator);
              }
#endif
              reinits++;
            }
