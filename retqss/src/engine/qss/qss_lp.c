...
LP_processPartition(QSS_simulator simulator, PRT_partition partition)
{
  ...
  LP_partitionInfo *pi = LP_PartitionInfoArray(lps, endStates - beginStates,
      endHandlers - beginHandlers, outputs);
  int i;
    ...
    for(j = 0; j < nOD; j++)
    {
      ...
      for(g = 0; g < nDH; g++)
      {
        ...
        LP_partitionInfo handlerPartition =
            pi[partition->values[partition->beginHandlers + h]];
...
LP_initializeDataStructs(QSS_simulator simulator, PRT_partition partition)
{
  ...
  for(i = 0; i < lps; i++)
  {
    simulator->lps->lp[i] = QSS_LP_Data(pi[i]->states, pi[i]->handlers, 0,
        pi[i]->outputs, pi[i]->stateInputs, pi[i]->handlerInputs, states,
        events,
    ...
    for(i = 0; i < lps; i++)
    {
      if(noComm[i] == TRUE)
      {
        ...
        SD_print(simulator->simulationLog,
            "LP%d: Controlling state variable: %d", i, asg);
      }
    }
  }
  autonomous = TRUE;
