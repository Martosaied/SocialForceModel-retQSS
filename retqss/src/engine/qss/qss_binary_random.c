    ...
    for(i = 1; i <= tree->height; i++)
    {
      for(j = 0; j <= numberOfNodes[i]; j++)
      {
        ...
        tree->nodes[i][j].visit[0] = MIN(tree->leaves[leftChild], MUL2(j),
            tree->leaves[rightChild], MUL2(j)+1);
        if(tree->leaves[leftChild] == tree->leaves[rightChild])
        {
...
void
BTR_updateTree(SC_scheduler scheduler, BTR_tree tree, int *inf, int cant,
    int idx, double *times)
{
  if(tree->size == 1)
    return;
  int vars, nodes;
  ...
  if(idx >= 0)
  {
    scheduler->state->visit->fathers[0][0] =
        tree->nodes[0][tree->father[idx]].father;
  ...
  {
    scheduler->state->visit->fathers[0][0] =
        tree->nodes[0][tree->father[inf[0]]].father;
  }
  vars = cant;
  for(i = 0; i < vars; i++)
  {
    ...
    tree->nodes[0][updateVar].value = minIdx;
    if(tree->nodes[0][updateVar].father
        != scheduler->state->visit->fathers[0][nodes - 1])
    {
      scheduler->state->visit->fathers[0][nodes] =
          tree->nodes[0][updateVar].father;
      nodes++;
    }
  }
  for(j = 1; j <= tree->height; j++)
  {
    ...
    for(i = 0; i < vars; i++)
    {
      ...
      {
        scheduler->state->visit->fathers[j][nodes] =
            tree->nodes[j][updateVar].father;
        nodes++;
      }
    }
  }
...
void
BTR_updateS(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  int w = ST_State, minIdx = -1, stateVar;
  double minVal = INF;
  stateVar = simTime->minIndex;
  BTR_updateTree(scheduler, scheduler->state->BTR_states, simData->SD[stateVar],
      simData->nSD[stateVar], stateVar, simTime->nextStateTime);
  minIdx =
      scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
...
void
BTR_updateH(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  int w = ST_State, minIdx = -1, stateVar;
  double minVal = INF;
  ...
  {
    stateVar = simTime->minIndex;
    BTR_updateTree(scheduler, scheduler->state->BTR_states,
        simData->SD[stateVar], simData->nSD[stateVar], stateVar,
        simTime->nextStateTime);
    minIdx =
        scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
    minVal = simTime->nextStateTime[minIdx];
    if(simData->nSZ[stateVar] && scheduler->state->BTR_events->size != 1)
      BTR_updateTree(scheduler, scheduler->state->BTR_events,
          simData->SZ[stateVar], simData->nSZ[stateVar], -1,
          simTime->nextEventTime);
    if(simTime->nextEventTime[scheduler->state->BTR_events->nodes[scheduler->state->BTR_events->height][0].value]
        < minVal)
    {
      minIdx =
          scheduler->state->BTR_events->nodes[scheduler->state->BTR_events->height][0].value;
  ...
  else if(simTime->type == ST_Event)
  {
    ...
    if(simData->nHD[stateVar])
      BTR_updateTree(scheduler, scheduler->state->BTR_states,
          simData->HD[stateVar], simData->nHD[stateVar], -1,
          simTime->nextStateTime);
    if(simData->event[stateVar].nLHSSt)
    {
      ...
      for(i = 0; i < simData->event[stateVar].nLHSSt; i++)
      {
        ...
        BTR_updateTree(scheduler, scheduler->state->BTR_states, simData->SD[j],
            simData->nSD[j], j, simTime->nextStateTime);
      }
    }
    minIdx =
        scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
    minVal = simTime->nextStateTime[minIdx];
    BTR_updateTree(scheduler, scheduler->state->BTR_events,
        simData->HZ[stateVar], simData->nHZ[stateVar], stateVar,
        simTime->nextEventTime);
    if(simTime->nextEventTime[scheduler->state->BTR_events->nodes[scheduler->state->BTR_events->height][0].value]
        < minVal)
    {
...
void
BTR_updateI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  int w = ST_State, minIdx = -1, stateVar;
  double minVal = INF;
  if(simTime->type == ST_State)
  {
    ...
    minIdx =
        scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
    minVal = simTime->nextStateTime[minIdx];
    if(simTime->nextInputTime[scheduler->state->BTR_inputs->nodes[scheduler->state->BTR_inputs->height][0].value]
        < minVal)
    {
      minIdx =
          scheduler->state->BTR_inputs->nodes[scheduler->state->BTR_inputs->height][0].value;
  ...
  {
    BTR_updateTree(scheduler, scheduler->state->BTR_states, NULL, 0,
        simData->TD[simTime->minIndex], simTime->nextStateTime);
    minIdx =
        scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
    minVal = simTime->nextStateTime[minIdx];
    BTR_updateTree(scheduler, scheduler->state->BTR_inputs, NULL, 0,
        simTime->minIndex, simTime->nextInputTime);
    if(simTime->nextInputTime[scheduler->state->BTR_inputs->nodes[scheduler->state->BTR_inputs->height][0].value]
        < minVal)
    {
      minIdx =
...
void
BTR_updateHI(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  int w = ST_State, minIdx = -1, stateVar;
  double minVal = INF;
  if(simTime->type == ST_State)
...
BTR_init(SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  ...
  int stHeight = 0;
  if(simData->states > 3)
	stHeight = (int) LOG2(simData->states) - 1;
  int dscHeight = 0;
  if(simData->events > 3)
    dscHeight = (int) LOG2(simData->events) - 1;
  int inHeight = 0;
  if(simData->inputs > 3)
    inHeight = (int) LOG2(simData->inputs) - 1;
  max_h = stHeight;
  ...
  scheduler->state->BTR_states = BTR_Tree(simData->states,
      simTime->nextStateTime, NULL);
  ...
  if(simData->events)
  {
    scheduler->state->BTR_events = BTR_Tree(simData->events,
        simTime->nextEventTime, simTime->weights);
  ...
  if(simData->inputs)
  {
    scheduler->state->BTR_inputs = BTR_Tree(simData->inputs,
        simTime->nextInputTime, NULL);
