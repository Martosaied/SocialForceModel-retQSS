...
QSS_generateWeights(QSS_simulator simulator)
{
  if(simulator->settings->debug & SD_DBG_VarChanges)
  {
    ...
    if(simulator->settings->debug & SD_DBG_Weights)
    {
      ...
      if(GRP_readGraph(simulator->output->name, simulator->data, &xadj, &adjncy,
          &edges, 1, &vwgt, &ewgt, 0, NULL) == GRP_ReadError)
      {
      ...
      if(GRP_readGraph(simulator->output->name, simulator->data, &xadj, &adjncy,
          &edges, 1, &vwgt, &ewgt, 1, &hevars) == GRP_ReadError)
      {
        fprintf(stderr, "Could not read generated graph files.\n");
        abort();
      }
...
QSS_SEQ_printSimulationLog(QSS_simulator simulator)
{
  ...
  if(nOutputs > 0)
  {
    ...
    for(j = 0; j < nOutputs; j++)
    {
      SD_print(simulator->simulationLog, "Variable %s changes: %d",
          simulator->output->variable[j].name, OUT_getSteps(simulator->log, j));
    }
  }
  ...
  SD_print(simulator->simulationLog, "Total time: %g ms",
...
QSS_SEQ_initialize(SIM_simulator simulate)
{
  ...
  // TODO - retQSS: changed size for sweeping scripts
  char logFile[512];
  ...
  if(QSS_stateLog_isFlagEnabled(simulator))
	  QSS_stateLog_init(simulator);
#endif
  ...
  if(QSS_stateLog_isFlagEnabled(simulator))
	  for(i = 0; i < forUL; i++)
		  QSS_stateLog_logState(i, simulator);
  if (settings->debug & SD_DBG_InitValues)
  {
  ...
  for(i = 0; i < forUL; i++)
  {
    QA_recomputeNextTime(quantizer, i, qssTime->time, qssTime->nextStateTime, x,
        qssData->lqu, q);
  }
#ifdef DEBUG
  if (settings->debug & SD_DBG_InitValues)
...
QSS_PAR_printSimulationLog(QSS_simulator simulator)
{
  ...
  SD_print(simulator->simulationLog, "Simulation past events: %lu (%.2f %%)",
      simulator->stats->pastEvents,
      (simulator->stats->extTrans > 0) ?
          ((double) simulator->stats->pastEvents
              / (double) simulator->stats->extTrans) * 100 :
          0);
  SD_print(simulator->simulationLog, "Dt sum: %g", simulator->dt->state->avgDt);
...
QSS_PAR_statistics(QSS_simulator simulator)
{
  ...
  SD_print(simulator->simulationLog, "External events: %lu",
      totalExternalEvents);
  SD_print(simulator->simulationLog, "Processed messages: %.2f %%",
      (totalMessages > 0) ?
...
QSS_PAR_logMemory(QSS_simulator simulator)
{
  ...
  for(i = 0; i < lps; i++)
  {
    ...
    simulator->stats->memory += (lp->totalStates + lp->totalEvents
        + lp->totalOutputs + lp->states + lp->events + lp->inputs + lp->outputs
        + lp->inStates + lp->inEvents) * sInt;
    ...
    QSS_internalEventHandler internalEvent)
{
  ...
  {
    QSS_PAR_sendSimultaneousMessages(simulator, externalEvent, internalEvent);
    if(QSS_SIS_empty(simulator->simSteps))
    {
