    ...
    NULL), _gen(NULL), _printer(NULL), _ri(NULL)
{
  ...
  _printer = newMMO_PrintExp(_data->symbols(), _ri, data->packages(), data);  
}
...
MMO_PrintExp_::MMO_PrintExp_(VarSymbolTable vt, MMO_ReplaceInterval ri,
    MMO_PackageTable pt, MMO_ModelData data) :
    _vt(vt), _idx(), _offset(0), _order(0), _constant(-1), _functions(), _variables(), _expressionOrder(
        1), _ri(ri), _pt(pt), _forOffset(), _in_function_call(false), _rid(0), _data(data), _fcc(0)
{
...
bool MMO_PrintExp_::checkOriginalExp(AST_Expression exp)
{
  AST_Expression_ComponentReference cr = exp->getAsComponentReference();
  AST_Expression orig_exp = cr->originalExp();
  if (orig_exp == nullptr) {
    return false;
  }
  AST_Expression_ComponentReference orig_cr_exp = orig_exp->getAsComponentReference();
  return !orig_cr_exp->hasIndexes();
}
...
MMO_PrintExp_::foldTraverseElement(AST_Expression exp)
{
  ...
  switch(exp->expressionType())
  {
    ...
    case EXPCOMPREF:
      ...
      if(_in_function_call && vi->isArray() && checkOriginalExp(exp)) { // && _fullArray[_rid].front()) {
    	  ret << "&(";
        ret << _vt->print(vi, _idx, _offset, _order, idxs, _constant, _forOffset, true);
        ret << ")";
        //_fullArray[_rid].pop_front();
      } else if(_data->outputArray() && vi->isArray() && checkOriginalExp(exp)) { 
        ret << _vt->print(vi, _idx, _offset, _order, idxs, _constant, _forOffset, true);
      } else {
    ...
    case EXPCALL:
      ...
      else
      {
        ...
        else if(bif == BIF_NONE)
        {
          ...
          if(pre.empty())
          {
            string local = _data->searchLocalPackageFuncs(name);
            if (local.empty()) {
               ret << "__"; 
            } else {
              ret << local;
...
MMO_PrintExp
newMMO_PrintExp(VarSymbolTable vt, MMO_ReplaceInterval ri, MMO_PackageTable pt, MMO_ModelData data)
{
  return new MMO_PrintExp_(vt, ri, pt, data);
}
...
MMO_ReplaceInterval_::MMO_ReplaceInterval_(VarSymbolTable vt) :
    _vt(vt), _indexes(0), _replacedVars(), _replacedExpsVars(), _replacedExps(), _fullArray(), _currentVar(), _rid(0), _in_function_call(false), _fcc(0)
{
}
