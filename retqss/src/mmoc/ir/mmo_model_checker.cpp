...
AST_MicroModelica_::visit(AST_Class x)
{
  ...
  {
    Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
        ER_Error, "Extend modifier to Class definition.");
  }
  if(_fatherName != NULL)
  {
    ...
    else
    {
      ...
      EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Only Function classes allowed.");
    }
  }
  else
  {
...
AST_MicroModelica_::visit(AST_Composition x)
{
  ...
  else if(_classPrefix == CP_MODEL)
  {
    ...
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "External function call inside Model class.");
    }
  }
  else if(_classPrefix == CP_PACKAGE)
  {
    ...
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Composition elements inside Package class.");
    }
    if(x->hasExternalFunctionCall())
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "External function call inside Package class.");
    }
  }
}
...
AST_MicroModelica_::visit(AST_CompositionElement x)
{
  if(_classPrefix == CP_MODEL)
  {
    ...
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Protected element definition inside Model Class.");
    }
  }
}
...
AST_MicroModelica_::visit(AST_CompositionEqsAlgs x)
{
  if(_classPrefix == CP_MODEL)
  {
    ...
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Initial Equation section inside Model Class.");
    }
  }
  if(_classPrefix == CP_FUNCTION || _classPrefix == CP_PURE
      || _classPrefix == CP_IMPURE)
  {
    ...
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Initial section inside function definition.");
    }
  }
}
...
AST_MicroModelica_::visit(AST_Element x)
{
  ...
  if(e == COMPONENT)
  {
    ...
    foreach(it,dl)
    {
          ...
          && !(tp & TP_CONSTANT))
      {
        if(current_element(it)->modification()->modificationType() == MODEQUAL)
        {
          Error::getInstance()->add(x->lineNum(),
          EM_AST | EM_CLASS_DEFINITION,
              ER_Error, "Only parameters or constants can be assigned.");
        }
      }
    }
    if(c->hasIndexes())
    {
      Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Array subscripts modifiers in component.");
    }
    if(c->isStream())
    {
      Error::getInstance()->add(x->lineNum(),
      EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Stream type prefixes not allowed.");
    }
    if(c->isFlow())
    {
      Error::getInstance()->add(x->lineNum(),
      EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Flow type prefixes not allowed.");
    }
    if(c->isConstant())
    {
      AST_DeclarationListIterator it;
        ...
        if(current_element(it)->hasModification())
        {
          ...
          if(m->modificationType() == MODCLASS)
          {
            ...
            if(asgExp->expressionType() == EXPNULL)
            {
              Error::getInstance()->add(x->lineNum(),
              EM_AST | EM_CLASS_DEFINITION,
                  ER_Error, "Wrong constant definition %s.", c->name().c_str());
            }
          }
        }
        else
    ...
    if(!Util::getInstance()->checkTypeString(c->type()))
    {
      Error::getInstance()->add(x->lineNum(),
      EM_AST | EM_CLASS_DEFINITION,
          ER_Error, "Predefined type not recognized.");
    }
  }
    ...
    else if(et == EXPOUTPUT)
    {
      ...
      foreach(it,el)
      {
        if(current_element(it)->expressionType() != EXPCOMPREF)
        {
          ...
          EM_AST | EM_CLASS_DEFINITION,
              ER_Error, "Wrong algebraic expression.");
          return false;
        }
      }
    }
  ...
  switch(et)
  {
    ...
    case EXPCOMPREF:
      ...
      if(el->size() > 1)
      {
        Error::getInstance()->add(x->lineNum(),
        EM_AST | EM_CLASS_DEFINITION,
            ER_Error, "Only array definitions allowed.");
      }
      if(el->size() > 0)
      {
        visit(el->front());
  ...
  if(x->argumentType() == AR_MODIFICATION)
  {
    ...
    if(am->hasModification() && _classModification == false)
    {
      if(Util::getInstance()->checkExperimentAnnotations(am->name()))
      {
        Error::getInstance()->add(x->lineNum(),
        EM_AST | EM_DEFINITION_NOT_ALLOWED,
            ER_Error, "Annotation not recognized.");
      }
    }
  }
}
...
AST_MicroModelica_::visit(AST_Statement x)
{
  ...
  switch(st)
  {
    ...
    case STRETURN:
      ...
      {
        Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
            ER_Error, "return statement outside function class.");
      }
      break;
    case STWHEN:
      {
      ...
      {
        Error::getInstance()->add(x->lineNum(), EM_AST | EM_CLASS_DEFINITION,
            ER_Error, "When condition must be a simple relation.");
      }
...
AST_MicroModelica_::visit(AST_StoredDefinition x)
{
  if(x->hasWithin())
  {
    Error::getInstance()->add(x->lineNum(),
    EM_AST | EM_DEFINITION_NOT_ALLOWED,
        ER_Error, "Within inside stored definition.");
  }
}
