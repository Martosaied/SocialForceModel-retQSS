...
MMO_Statement_::_init()
{
  ...
  switch(_stm->statementType())
  {
    ...
    case STOUTASSING: {
      ...
      foreach(it, out_stm->out_expressions()) {
        AST_Expression exp = current_element(it);
        if (exp->expressionType() != EXPCOMPREF) {
          Error::getInstance()->add(_stm->lineNum(),
          EM_IR | EM_VARIABLE_NOT_FOUND,
              ER_Error, "Wrong expression type, must be a variable definition");
          return;
        }
        ...
        if (!cr->hasIndexes() && vi->isArray()) {
          idx.setFactor(1);
          _lhs->insert(idx, DEP_DISCRETE_VECTOR);
        } else {
          _lhs->insert(idx, DEP_DISCRETE);
        }
        _insertDeps(exp);
    ...
    int forOffset)
{
  ...
  switch(_stm->statementType())
  {
    ...
    case STOUTASSING: {
    AST_Statement_OutputAssigment out_stm = _stm->getAsOutputAssigment();
    AST_Expression call_stm = newAST_Expression_Call(newAST_String(out_stm->function()->name()), NULL, out_stm->arguments());
    MMO_Expression call_exp = newMMO_Expression(call_stm, _data);
    string call_str = call_exp->print("i", offset, order, -1, 1, forOffset);
    call_str.pop_back();
    buffer << call_str;
    AST_ExpressionListIterator it;
    int size = out_stm->out_expressions()->size(), i = 0;
    if (out_stm->arguments()->size() > 0) {
      buffer << ", ";
    }
    foreach(it,out_stm->out_expressions())
    {
      i++;
      MMO_Expression arg = newMMO_Expression(current_element(it), _data);
      if (arg->exp()->expressionType() == EXPCOMPREF) {
        AST_Expression_ComponentReference cr = arg->exp()->getAsComponentReference();
        if (!_data->isFunction() || !_data->isFunctionOutputVariable(cr->name())) {
          buffer << "&"; 
        }
      }
      _data->setOutputArray(true);
      buffer << "(" << arg->print(idx, offset, 0, -1, 1, forOffset) << ")";
      _data->setOutputArray(false);
      buffer << (i < size ? ", " : "");
    }
    buffer << ");" << endl;
