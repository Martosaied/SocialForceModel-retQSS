...
MMO_ModelData_::MMO_ModelData_() :
    ...
    NULL), _disableSymDiff(false), _output_function_symbols(NULL), _is_function(false), _local_package_funcs(), _function_prefix("")
{
}
...
void
MMO_ModelData_::setOutputVariables(VarSymbolTable outputVariables)
{
  _output_function_symbols = outputVariables;
}

bool
MMO_ModelData_::isFunctionOutputVariable(VarName name)
{
  if (_output_function_symbols == NULL) {
    return false;
  }
  if (_output_function_symbols->count() == 1) {
    return false;
  } 
  VarInfo var =_output_function_symbols->lookup(name);
  if (var != NULL) {
    return true;
  }
  return false;
...
bool
MMO_ModelData_::isFunction() { return _is_function; }

void
MMO_ModelData_::setFunction(bool func) { _is_function = func; }

string
MMO_ModelData_::searchLocalPackageFuncs(string name) 
{ 
  if(_local_package_funcs.find(name) != _local_package_funcs.end()) {
    return _function_prefix;
  }
  return ""; 
}
