...
MMO_Equation_::jacobianExp(Index idx, DEP_Type type)
{
  ...
  map<string, MMO_Expression>::iterator f = _jacobianExps.find(
      _exp[0]->deps()->identifier(idx, type));
  ...
  {
    AST_Expression derExpAST = newAST_Expression_BinOp(f->second->exp(),
        chExp->exp(), BINOPADD);
    ...
    bool variableChange)
{
  ...
  else
  {
    buffer << indent << idx << " = " << lhs.printReverseDefinition(variable)
        << ";" << endl;
    buffer << indent << "if (" << idx << " >= " << lhs.begin() << " && " << idx
        << " <= " << lhs.end() << ")" << endl;
    buffer << indent << "{";
  }
  return buffer.str();
...
list<string>
MMO_Equation_::print(string indent, string eq_lhs, string idx, bool palgs,
    ...
    int forOffset, int constantOffset)
{
  ...
  list<string> ret;
  string lhs = "";
  if (_data->isFunctionOutputVariable(eq_lhs)) {
    lhs = "*";  
  }
  lhs.append(eq_lhs);
  if(palgs)
  {
        ...
        dIdx = d->next(DEP_ALGEBRAIC))
    {
      if(d->isVector(dIdx))
      {
        ...
        {
          if(controlAlgebraicArguments(&algebraicArguments,
              (*eq)->algebraicArguments()))
          {
            ...
            if(lhsEq.hasRange())
            {
              ...
              buffer << indent << "for(" << iter << " = " << lhsEq.begin()
                  << "; " << iter << " <= " << lhsEq.end() << "; " << iter
                  << "++)";
            ...
            list<string> a = (*eq)->print(indent + indent, aLhs.str(),
                Util::getInstance()->getVarName(iter), false, algs,
                EQ_ALGEBRAIC, order, false, offset, false, forOffset,
                -1);
              ...
              (*eq)->algebraicArguments()))
          {
            ...
            else
            {
              if(dIdx->factor() != 1 || dIdx->operConstant() != 0)
              {
                ...
                variableMod = true;
                aLhs << algPrefix << "[("
                    << (*eq)->lhs().print(dIdx->definition(vn),
                        -(*eq)->lhs().operConstant() + offset, false) << ")";
            ...
            list<string> a = (*eq)->print(indent, aLhs.str(), strIdx, false,
                algs, EQ_ALGEBRAIC, order, constantValue, offset, false,
                forOffset,
                cte);
            list<string> vars = (*eq)->getVariables();
            _variables.insert(_variables.end(), vars.begin(), vars.end());
...
MMO_Equation_::controlAlgebraicDefinition()
{
      ...
      !deps->end(DEP_ALGEBRAIC_DEF); idx = deps->next(DEP_ALGEBRAIC_DEF))
  {
    ...
    if(eqs.empty())
    {
      Error::getInstance()->add(_exp[0]->exp()->lineNum(), EM_IR | EM_ALG_INDEX,
          ER_Error, "%s and %s.", _lhs.print("i").c_str(),
          idx->print("i").c_str());
    }
  }
...
MMO_Equation_::_generateChainRule(Index idx)
{
  ...
  for(Index *dIdx = deps->begin(DEP_ALGEBRAIC_DEF);
      !deps->end(DEP_ALGEBRAIC_DEF); dIdx = deps->next(DEP_ALGEBRAIC_DEF))
  {
    map<string, MMO_Expression>::iterator af = _jacobianExps.find(
        _exp[0]->deps()->identifier(*dIdx, DEP_ALGEBRAIC_DEF));
    ...
    for(eq = eqs.begin(); eq != eqs.end(); eq++)
    {
      map<string, MMO_Expression>::iterator f = (*eq)->_jacobianExps.find(
          (*eq)->_exp[0]->deps()->identifier(idx, DEP_STATE));
      MMO_Expression derExp =
          (f == (*eq)->_jacobianExps.end()) ? NULL : f->second;
      if(derExp != NULL)
      {
        if(algExp == NULL)
        {
        ...
        {
          AST_Expression derAST = newAST_Expression_BinOp(algExp->exp(),
              derExp->exp(), BINOPADD);
    ...
    if(algExp != NULL)
    {
      AST_Expression chain = newAST_Expression_BinOp(af->second->exp(),
          algExp->exp(), BINOPADD);
