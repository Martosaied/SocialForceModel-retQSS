...
#define CREF_NAME(X)                                                           \
  (IS_UMINUS(X)                                                                \
       ? *(UMINUS_EXP(X)->getAsComponentReference()->names()->front())         \
       : *((X)->getAsComponentReference()->names()->front()))
/**
 *
 */
 ...
 *
 */
#define IS_ZERO_REAL(X)                                                        \
  ((X)->expressionType() == EXPREAL && (X)->getAsReal()->val() == 0.0)
/**
 *
 */
#define IS_ZERO_INT(X)                                                         \
  ((X)->expressionType() == EXPINTEGER && (X)->getAsInteger()->val() == 0)
/**
 *
 */
 ...
 */
#define IS_ADD(X)                                                              \
  ((X)->expressionType() == EXPBINOP &&                                        \
   (X)->getAsBinOp()->binopType() == BINOPADD)
/**
 *
 */
...
#define IS_SUB(X)                                                              \
  ((X)->expressionType() == EXPBINOP &&                                        \
   (X)->getAsBinOp()->binopType() == BINOPSUB)
/**
 *
 */
 ...
 */
#define IS_STATE(X)                                                            \
  (_varSymbolTable->lookup(CREF_NAME(X)) != NULL &&                            \
   _varSymbolTable->lookup(CREF_NAME(X))->isState())
/**
 *
 */
#define IS_PARAMETER(X)                                                        \
  (IS_UMINUS(X)                                                                \
       ? _varSymbolTable->lookup(CREF_NAME(UMINUS_EXP(X))) != NULL &&          \
             _varSymbolTable->lookup(CREF_NAME(UMINUS_EXP(X)))->isParameter()  \
       : _varSymbolTable->lookup(CREF_NAME(X)) != NULL &&                      \
             _varSymbolTable->lookup(CREF_NAME(X))->isParameter())
/**
 *
 */
#define IS_RELOP(X)                                                            \
  ((X)->expressionType() == EXPBINOP &&                                        \
   (X)->getAsBinOp()->binopType() >= BINOPLOWER &&                             \
   (X)->getAsBinOp()->binopType() <= BINOPGREATEREQ)
/**
 *
 */
 ...
 *
 */
template <class R> class AST_Expression_Visitor {
public:
  /**
   *
   */
 ...
 *
 */
template <class R> class AST_Expression_Fold {
public:
  /**
   *
   */
...
private:
  ...
  bool compareArrays(AST_Expression_ComponentReference arrayA,
 ...
 * @param m
 */
void deleteControlVars(ControlVars m);
...
public:
   ...
   * @return
   */
  AST_Expression replaceExp(AST_Expression rep, AST_Expression for_exp,
 ...
 * TODO Ver el calculo del tipo de la expression de retorno en el caso en el que
 * se llega a obtener un número.
 */
/**
 *
 */
...
public:
  ...
  /**
   * Evaluates an arithmetic expression replacing the occurrences of compRef
   * with the value provided (compRefValue).
   */
  /**
   *
   * @param compRef
   ...
   * @return
   */
  AST_Expression eval(AST_Expression_ComponentReference compRef,
...
public:
   ...
   * @param index
   */
  void setIndex(Index *idx, Index index, int dim = 0);

private:
  Index foldTraverseElement(AST_Expression);
  Index foldTraverseElementUMinus(AST_Expression);
  Index foldTraverseElement(Index, Index, BinOpType);
  VarSymbolTable _vt;
  int _binops;
};
/**
 ...
 * @param m
 */
void deleteExpressionIndex(ExpressionIndex m);
...
private:
  ...
  MMO_ModelData _data;
  Index _idx;
  Dependencies foldTraverseElement(AST_Expression exp);
  Dependencies foldTraverseElementUMinus(AST_Expression exp);
  Dependencies foldTraverseElement(Dependencies l, Dependencies r,
                                   BinOpType bot);
  void _algebraicsDeps(AST_Expression_ComponentReference cr, Index idx,
 ...
 * @param m
 */
void deleteGenerateDeps(GenerateDeps m);
...
public:
   ...
   * @param offset
   */
  void setEnvironment(string idx, bool modelica = false, int offset = 1);

private:
  string foldTraverseElement(AST_Expression exp);
  string foldTraverseElementUMinus(AST_Expression exp);
  string foldTraverseElement(string l, string r, BinOpType bot);
  VarSymbolTable _vt;
  string _idx;
  int _offset;
  bool _modelica;
 ...
 * @return
 */
ExpIndexes newExpIndexes(VarSymbolTable vt);
/**
 *
 * @param m
 */
void deleteExpIndexes(ExpIndexes m);
...
private:
  ...
  AST_Expression foldTraverseElement(AST_Expression l, AST_Expression r,
                                     BinOpType bot);
  VarSymbolTable _vt;
};
/**
 ...
 * @return
 */
ReplaceDer newReplaceDer(VarSymbolTable vt);
/**
 *
 * @param m
 */
void deleteReplaceDer(ReplaceDer m);
