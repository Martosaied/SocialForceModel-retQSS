...
VarSymbolTable_::print(VarInfo vi, string idx, int offset, int order,
    list<Index> idxs, int constant, int forOffset, bool full_array)
{
  stringstream ret;
  string idxStr;
  if(vi->isForType())
  ...
  if (full_array) {
    stringstream print_offset; 
    print_offset << "[" << vi->index().offset();
    if (vi->isState()) {
      print_offset << " * " << _coeffs;
    }
    print_offset << "]";
    idxStr = print_offset.str();
  } else if(vi->hasEachModifier() && (_vste == VST_INIT || _vste == VST_CLASSIC_INIT))
  {
    stringstream tmp;
    tmp << "[" << idx;
    if(!vi->isParameter())
...
MMO_EquationTable_::findGenericDependencies(int val)
{
  ...
  for(int i = 0; i < s; i++)
  {
    ...
    if(idx.factor() != 0)
    {
      ...
      for(d->begin(DEP_ALGEBRAIC_STATE); !d->end(DEP_ALGEBRAIC_STATE);
...
{
  for(MMO_PackageData pd = ListTable_<MMO_PackageData>::begin();
      !ListTable_<MMO_PackageData>::end(); pd =
          ListTable_<MMO_PackageData>::next())
  {
    ...
    for(MMO_FunctionDefinition fd = fdt->begin(); !fdt->end(); fd = fdt->next())
    {
      ...
      if(fd->name().compare(name) == 0
          || fd->name().compare(qualifiedName) == 0)
      {
        return pd->prefix();
      }
    }
