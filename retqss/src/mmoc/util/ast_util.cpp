...
AST_Expression AST_Expression_Traverse::mapTraverse(AST_Expression e) {
  ...
  case EXPIF: {
    ...
    return (newAST_Expression_If(mapTraverse(i->condition()),
                                 mapTraverse(i->then()), i->elseif_list(),
...
bool EqualExp::equalTraverseElement(AST_Expression a, AST_Expression b) {
  ...
  case EXPDERIVATIVE: {
    ...
    ERROR_UNLESS(argsA->size() == 1, "EqualExp::equalTraverseElement:\n"
                                     "AST_Expression_Derivative with more than "
                                     "1 argument are not supported yet.\n");
...
VarInfo EqualExp::getVarInfo(AST_Expression_ComponentReference compRef) {
  ...
  if (names->size() > 0) {
    ERROR_UNLESS(names->size() == 1, "EqualExp::getVarInfo\n"
                                     "AST_Component_Reference with names list "
                                     "bigger than 1 are not supported yet.\n");
...
AST_Expression ReplaceExp::replaceExp(AST_Expression rep,
                                      AST_Expression for_exp, AST_Expression in,
...
AST_Expression ReplaceReference::foldTraverseElement(AST_Expression b1,
                                                     AST_Expression b2,
                                                     BinOpType t) {
  return newAST_Expression_BinOp(b1, b2, t);
}
...
AST_Expression EvalExp::eval(AST_Expression exp) { return foldTraverse(exp); }

AST_Expression EvalExp::eval(AST_Expression_ComponentReference compRef,
  ...
  case EXPDERIVATIVE: {
    ...
    foreach (argsIter, arguments) {
      ...
      ERROR_UNLESS(arg->expressionType() == EXPCOMPREF,
                   ...
                   "Incorrect AST_ExpressionDerivative argument type or not "
                   "supported yet.\n");
...
AST_Expression EvalExp::foldTraverseElement(AST_Expression left,
                                            AST_Expression right,
                                            BinOpType binOpType) {
  switch (binOpType) {
  case BINOPADD:
    if (shouldReturnInteger(left, right)) {
      return (newAST_Expression_Integer(left->getAsInteger()->val() +
...
bool ControlVars_::foldTraverseElement(AST_Expression e) {
...
bool ControlVars_::foldTraverseElement(bool e1, bool e2, BinOpType bot) {
  return e1 && e2;
}

bool ControlVars_::foldTraverseElementUMinus(AST_Expression e) {
  return foldTraverseElement(e);
}

ControlVars newControlVars(VarSymbolTable st, VarSymbolTable lst) {
  return new ControlVars_(st, lst);
}

void deleteControlVars(ControlVars m) { delete m; }

/* ExpressionIndex class. */

ExpressionIndex_::ExpressionIndex_(VarSymbolTable vt) : _vt(vt), _binops(0) {}

ExpressionIndex_::~ExpressionIndex_() {}

Index ExpressionIndex_::index(AST_Expression exp) {
...
void ExpressionIndex_::setIndex(Index *idx, Index index, int dim) {
...
Index ExpressionIndex_::foldTraverseElement(AST_Expression exp) {
  Index ret = Index();
  switch (exp->expressionType()) {
  case EXPCOMPREF: {
    ...
    if (vi == NULL) {
      Error::getInstance()->add(exp->lineNum(), EM_IR | EM_VARIABLE_NOT_FOUND,
    ...
    if (vi->isForType()) {
      ...
      if (cr->hasIndexes()) {
        ...
        if (parameterIndex.factor() != 0) {
          ...
          Error::getInstance()->add(exp->lineNum(),
                                    EM_IR | EM_WRONG_VARIABLE_TYPE, ER_Error,
                                    "%s", cr->name().c_str());
        }
      }
      ...
      Error::getInstance()->add(exp->lineNum(), EM_IR | EM_WRONG_VARIABLE_TYPE,
  ...
  default:
    Error::getInstance()->add(exp->lineNum(), EM_IR | EM_WRONG_EXP, ER_Error,
                              "Expression index.");
    break;
  }
  return ret;
...
Index ExpressionIndex_::foldTraverseElementUMinus(AST_Expression exp) {
...
Index ExpressionIndex_::foldTraverseElement(Index l, Index r, BinOpType bot) {
  Index ret;
  ...
  case BINOPMULT:
    ...
    ret.setConstant(r.operConstant() * l.operConstant() + factor);
    break;
  case BINOPDIV:
    if (l.factor() != 0 && r.factor() != 0) {
      Error::getInstance()->add(0, EM_IR | EM_WRONG_EXP, ER_Error,
  ...
  case BINOPADD:
    ...
    ret.setConstant(l.operConstant() + r.operConstant());
    if (_binops > 0) {
      ret.setConstant(ret.operConstant() - 1);
    }
    break;
  case BINOPSUB:
    ret.setFactor(l.factor() - r.factor());
...
ExpressionIndex newExpressionIndex(VarSymbolTable vt) {
  return new ExpressionIndex_(vt);
}

void deleteExpressionIndex(ExpressionIndex m) { delete m; }
...
void GenerateDeps_::_algebraicsDeps(AST_Expression_ComponentReference cr,
                                    Index idx, VarInfo vi, Dependencies ret) {
  list<MMO_Equation> eqs = _algs->equation(vi->index());
  if (eqs.empty()) {
    Error::getInstance()->add(cr->lineNum(), EM_IR | EM_VARIABLE_NOT_FOUND,
...
Dependencies GenerateDeps_::foldTraverseElement(AST_Expression exp) {
  ...
  case EXPCOMPREF: {
    ...
    string identifier = cr->name();
    ExpIndexes_ ei(_vt);
    ei.setEnvironment("i", true);
    identifier += ei.foldTraverse(cr);
    if (vi->isState() || vi->isAlgebraic() || vi->isDiscrete()) {
      ...
      if (cr->hasIndexes()) {
        ...
        if (vi->isState()) {
          ...
          ret->insert(idx, identifier, DEP_STATE);
  ...
  case EXPCALL: {
      ...
      for (MMO_Function f = ft->begin(); !ft->end(); f = ft->next()) {
        if (!f->name().compare(name)) {
          ...
          if (_data->disableSymDiff()) {
            ...
            Error::getInstance()->add(0, EM_CG, ER_Warning,
                                      "External functions detected, set "
                                      "symbolic differentiation off.");
          }
          break;
        }
      }
    ...
    if (notFound && !_data->initialCode()) {
      Error::getInstance()->add(exp->lineNum(), EM_IR | EM_FUNCTION_NOT_FOUND,
...
Dependencies GenerateDeps_::foldTraverseElementUMinus(AST_Expression exp) {
  return foldTraverse(exp->getAsUMinus()->exp());
}

Dependencies GenerateDeps_::foldTraverseElement(Dependencies l, Dependencies r,
                                                BinOpType bot) {
  l->join(r);
  return l;
}

GenerateDeps newGenerateDeps(MMO_ModelData data) {
  return new GenerateDeps_(data);
}

void deleteGenerateDeps(GenerateDeps m) { delete m; }
...
string ExpIndexes_::foldTraverseElement(AST_Expression exp) {
...
string ExpIndexes_::foldTraverseElementUMinus(AST_Expression exp) {
  return foldTraverse(exp->getAsUMinus()->exp());
}

string ExpIndexes_::foldTraverseElement(string l, string r, BinOpType bot) {
  stringstream ret;
  if (!l.empty() && !r.empty()) {
    ret << l << "," << r;
...
void ExpIndexes_::setEnvironment(string idx, bool modelica, int offset) {
  _idx = idx;
  _modelica = modelica;
  _offset = offset;
}

ExpIndexes newExpIndexes(VarSymbolTable vt) { return new ExpIndexes_(vt); }

void deleteExpIndexes(ExpIndexes m) { delete m; }

ReplaceDer_::ReplaceDer_(VarSymbolTable vt) : _vt(vt) {}

ReplaceDer_::~ReplaceDer_() {}

AST_Expression ReplaceDer_::foldTraverseElement(AST_Expression exp) {
  stringstream ret(stringstream::out);
  switch (exp->expressionType()) {
  case EXPCALL: {
    ...
    if (!name.compare("der2")) {
      return (newAST_Expression_Derivative(newAST_ExpressionList(
          newAST_Expression_Derivative(ec->arguments()))));
    } else if (!name.compare("der3")) {
      return (newAST_Expression_Derivative(newAST_ExpressionList(
          newAST_Expression_Derivative(newAST_ExpressionList(
              newAST_Expression_Derivative(ec->arguments()))))));
    }
  } break;
  case EXPOUTPUT:
    return (newAST_Expression_OutputExpressions(newAST_ExpressionList(
        foldTraverse(AST_ListFirst(exp->getAsOutput()->expressionList())))));
  default:
    break;
  }
  return exp;
...
AST_Expression ReplaceDer_::foldTraverseElementUMinus(AST_Expression exp) {
  return newAST_Expression_UnaryMinus(foldTraverse(exp->getAsUMinus()->exp()));
}

AST_Expression ReplaceDer_::foldTraverseElement(AST_Expression l,
                                                AST_Expression r,
                                                BinOpType bot) {
  return newAST_Expression_BinOp(l, r, bot);
}

ReplaceDer newReplaceDer(VarSymbolTable vt) { return new ReplaceDer_(vt); }

void deleteReplaceDer(ReplaceDer m) { delete m; }
