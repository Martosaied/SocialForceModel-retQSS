...
string Util::removeExtraSlash(string path)
{
  if (path.empty()) {
    return path;
  }
  string ret = path;
  string sub = ret.substr(ret.length()-1,ret.length());
  if (sub.compare("/") == 0) {
...
Util::packagePath(string pname, MMO_CompileFlags flags, string ext)
{
  ...
  {
    f.close();
    return removeExtraSlash(flags->path());
  }
  else
  {
    f.close();
    if(flags->hasOutputFile())
    {
      ...
      {
        f.close();
        return removeExtraSlash(flags->outputFilePath());
      }
      f.close();
    }
