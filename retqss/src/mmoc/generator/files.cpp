...
MMO_Files_::makefile()
{
  ...
  buffer << " $(TARGET_SRC) $(CFLAGS) -c -o $(TARGET).o\n";
  buffer << _writer->indent(1) << "$(CXX) $(INC)";
  if(_flags->hasObjects())
  {
    buffer << " $(OBJ)";
  }
  buffer << " $(TARGET).o $(CFLAGS) -o $@ -lm -lgsl -lconfig -lgfortran";
#ifdef	__linux__
  // TODO: retQSS
  buffer
      << " -lpthread -lmetis -lscotch -lscotcherr -lpatoh -lrt -lsundials_cvode -lsundials_ida -lsundials_nvecserial -lCGAL -ldl"; //-llapack -latlas -lf77blas -lklu";
#endif
