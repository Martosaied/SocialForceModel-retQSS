    ...
    int assignments, DEP_Type type)
{
  ...
  bool vector = (type == DEP_STATE_VECTOR || type == DEP_DISCRETE_VECTOR);
  string for_iter = "i0";
  string for_rhs_idx = "i";
  if (vector) {
    for_iter = "j0"; 
    for_rhs_idx = "j";
  ...
  switch(intersection.type())
  {
    ...
    case IDX_EQUAL:
      if(idx.hasRange())
      {
        buffer << "for(" << for_iter << " = " << idx.mappedBegin() << "; " << for_iter << " <= "
            << idx.mappedEnd() << "; " << for_iter << "++)";
        ...
        else
        {
          ...
          buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
              << infIdx.print(for_rhs_idx, -infIdx.begin()) << ";";
          _writer->write(&buffer, init);
          buffer << "}";
          _writer->write(&buffer, alloc, false);
          _writer->write(&buffer, init);
      ...
      else
      {
        ...
        if(infIdx.hasRange())
        {
          buffer << "for(" << for_iter << " = " << infIdx.begin() << "; " << for_iter << " <= " << infIdx.end()
              << "; " << for_iter << "++)";
          _writer->write(&buffer, init);
          buffer << "{";
          _writer->write(&buffer, init);
          buffer << indent << initStr << "[" << idxStr << "][" << counter << "["
              << idxStr << "]++] = " << infIdx.print(for_rhs_idx, -infIdx.begin())
              << ";";
          _writer->write(&buffer, init);
          buffer << "}";
          _writer->write(&buffer, init);
    ...
    case IDX_SUBSET:
      ...
      buffer << "for(" << for_iter << " = " << begin << "; " << for_iter << " <= "
          << begin + infDIdx->range() - 1 << "; " << for_iter << "++)";
      _writer->write(&buffer, alloc, false);
      _writer->write(&buffer, init);
      buffer << "{";
      _writer->write(&buffer, alloc, false);
      ...
      _writer->write(&buffer, alloc);
      buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
          << tmp.print(for_rhs_idx, -begin + infIdx.begin() - idx.begin()) << ";";
      _writer->write(&buffer, init);
      buffer << "}";
      _writer->write(&buffer, alloc, false);
      _writer->write(&buffer, init);
    ...
    case IDX_SUBSET_OF:
      ...
      _writer->write(&buffer, alloc);
      buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
          << tmp.print(for_rhs_idx, -idx.mappedBegin() + idx.begin()) << ";";
      _writer->write(&buffer, init);
      buffer << "}";
      _writer->write(&buffer, alloc, false);
      _writer->write(&buffer, init);
    ...
    case IDX_ARB_BA:
      ...
      _writer->write(&buffer, alloc);
      buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
          << infIdx.print(for_rhs_idx, -begin) << ";";
      _writer->write(&buffer, init);
      buffer << "}";
      _writer->write(&buffer, alloc, false);
      _writer->write(&buffer, init);
      ...
      break;
    // IDX_CONSTANT_BA case should be handled as a vector dependency.
    case IDX_CONSTANT_AB:
      {
        int cte = idx.mappedConstant();
        ...
        buffer << indent << allocStr << "[" << idx_str.str() << "]++;";
        _writer->write(&buffer, alloc);
        buffer << indent << initStr << "[" << idx_str.str() << "][" << counter << "[" << idx_str.str() << "]++] = "
            << infIdx.print("i", intersection.value() - offset) << ";";
        _writer->write(&buffer, init);
        if (!vector) {
          buffer << "}";
          _writer->write(&buffer, alloc, false);
...
bool
QSS_::_hasPrevIntersections(Index event_def, map<Index, Index> events, Intersection new_intersection,
        list<Intersection> local_intersections)
{
  if (events.find(event_def) == events.end()) {
    return false;
  }
  list<Intersection>::iterator it;
  for (it = local_intersections.begin(); it != local_intersections.end(); it++) {
    if (it->equal(new_intersection)) {
      return true;
    }
  }
  return false;
    ...
    map<Index, Index> events)
{
  ...
  DEP_Type zc_type = type;
  stringstream begin_loop;
  stringstream end_loop;
  map<Index, list<Intersection> > local_events_deps;
  if (type == DEP_STATE_VECTOR) {
    zc_type = DEP_STATE;
  } else if (type == DEP_DISCRETE_VECTOR) {
    zc_type = DEP_DISCRETE;
  }
      ...
      hndIdx = deps->next(type))
  {
    ...
    for(int eNum = 0; eNum < evCount; eNum++)
    {
      ...
      for(Index *zcIdx = zcDeps->begin(zc_type); !zcDeps->end(zc_type);
          zcIdx = zcDeps->next(zc_type))
      {
        _setInterval(zcIdx, &ecIndex);
        Intersection is = hndIdx->intersection(*zcIdx);
        if(is.type() != IDX_DISJOINT)
        {
          if (_hasPrevIntersections(ecIndex, events, is, local_events_deps[*zcIdx])) {
            continue;
          }
          events[ecIndex] = ecIndex;
          local_events_deps[*zcIdx].push_back(is);
          if (type == DEP_STATE_VECTOR || type == DEP_DISCRETE_VECTOR) {
            begin_loop << "for(i0 = " << hndIdx->begin() << "; i0 <= " << hndIdx->end() << "; i0++) {";
            _writer->write(&begin_loop, WR_ALLOC_LD_HZ, false);
            _writer->write(&begin_loop, WR_INIT_LD_HZ);
          }    
        }
        ...
        if(is.type() != IDX_DISJOINT)
        {
          if (type == DEP_STATE_VECTOR || type == DEP_DISCRETE_VECTOR) {
            end_loop << "}" << endl;
            _writer->write(&end_loop, WR_ALLOC_LD_HZ, false);
            _writer->write(&end_loop, WR_INIT_LD_HZ);
          }    
        }
      }
    ...
    WR_Section init, string allocString, string initString, DEP_Type type)
{
  ...
  stringstream event_matrix; 
  event_matrix << "event[" << eqsIdx << "]."; 
  if(numDeps)
  {
    buffer << indent << "modelData->" << event_matrix.str()  << allocString
        << " += " << numDeps << ";";
    _writer->write(&buffer, alloc);
  }
  ...
  {
    Index dIdx(*idx);
    string sIdx = dIdx.print("j");
    buffer << "for(" << iter << " = " << idx->begin() << "; " << iter
        << " <= " << idx->end() << "; " << iter << "++)";
    _writer->write(&buffer, init);
    _writer->write("{", init);
    buffer << indent << "modelData->" << event_matrix.str() << initString
        << "[events[" << eqsIdx << "]++] = " << sIdx << ";";
    _writer->write(&buffer, init);
    _writer->write("}", init);
...
QSS_::initializeMatrices()
{
  ...
  for(MMO_Equation e = et->begin(); !et->end(); e = et->next())
  {
        ...
        dIdx = deps->next(DEP_DISCRETE))
    {
      ...
      for(MMO_Event ev = evt->begin(); !evt->end(); ev = evt->next())
      {
        ...
        for(Index *eIdx = eDeps->begin(DEP_DISCRETE_VECTOR); !eDeps->end(DEP_DISCRETE_VECTOR);
            eIdx = eDeps->next(DEP_DISCRETE_VECTOR))
        {
          _setInterval(eIdx, &eIndex);
          Intersection is = eIdx->intersection(*dIdx);
          if(is.type() == IDX_DISJOINT)
          ...
          stringstream begin_loop;
          stringstream end_loop;
          begin_loop << "for(i0 = " << eIdx->begin() << "; i0 <= " << eIdx->end() << "; i0++) {";
          end_loop << "}";
          _writer->write(&begin_loop, WR_ALLOC_LD_HD, false);
          _writer->write(&begin_loop, WR_INIT_LD_HD);
          _indexDependencies(eIndex, eIdx, equationIndex, dIdx, &simpleHDDeps,
              WR_ALLOC_LD_HD, WR_INIT_LD_HD, "modelData->nHD", "modelData->HD",
              "events", is, assignments);
          _writer->write(&end_loop, WR_ALLOC_LD_HD, false);
          _writer->write(&end_loop, WR_INIT_LD_HD);    
          HD[eIndex].push_back(is);
        }
      }
    }
  ...
  for(MMO_Event e = evt->begin(); !evt->end(); e = evt->next())
  {
    ...
    _eventDeps(e, index, evt, DEP_DISCRETE_VECTOR, eventDependencies);
    ...
    _eventDeps(e, index, evt, DEP_STATE_VECTOR, eventDependencies);
...
Classic_::makefile(SOL_Makefile m)
{
  ...
  switch(m)
  {
    case SOL_LIBRARIES:
      if(_flags->debug())
      {
        return "LIBS    := -lqssd -ltimestepd -lretqssd";
      }
      else
      {
        return "LIBS    := -lqss -ltimestep -lretqss";
      }
    case SOL_INCLUDES:
      return ("INC    := -I"
