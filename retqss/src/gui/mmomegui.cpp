...
void
MmomeGui::_enableActions(bool f)
{
  ...
  actionCompileAndRun->setEnabled(f);
  actionRun->setEnabled(f);
  actionSettings->setEnabled(f);
  actionAbort->setEnabled(!f);
...
MmomeGui::_addToolBarItems()
{
  ...
  _model_toolbar->addSeparator();
  _model_toolbar->addAction(actionCompileAndRun);
  _model_toolbar->addAction(actionRun);
  _model_toolbar->addAction(actionAbort);
...
void
MmomeGui::done(QString name, QString ext)
{
...
void
MmomeGui::closeEvent(QCloseEvent * event)
{
  QMdiSubWindow *sw = mdiArea->currentSubWindow();
  if(sw)
  {
...
{
  QString fileName = QFileDialog::getOpenFileName(this, tr("Load SBML Model"),
      _utils->appDir(MMOC_MODELS), "XML *.xml (*.xml)");
  if(fileName.isNull())
  {
    return;
  }
  ...
  _compiler_msg->setPlainText(
      _compiler_msg->toPlainText() + "\n" + _utils->appDir(MMOC_BIN) + SLASH
          + sim + " " + _sbmlFile + " " + fileName);
}
...
MmomeGui::_editModel(QString name)
{
  ...
  else
  {
    ...
    connect(Editor::instance(), SIGNAL(done(QString, QString)), this,
        SLOT(done(QString, QString)));
...
MmomeGui::_run_finished(int exitCode, QProcess::ExitStatus exitStatus)
{
  if(exitStatus == QProcess::NormalExit && exitCode == 0)
  {
    ...
    _compiler_msg->setPlainText(
        _compiler_msg->toPlainText() + QString("\n") + logs);
    _compiler_msg->setPlainText(
        _compiler_msg->toPlainText() + QString("Simulation ended."));
  ...
  if(exitStatus == QProcess::NormalExit && _runDlg->shouldGenerateDebugPlots())
    _generateDebugPlots();
  delete _runDlg;
}

void
MmomeGui::_settingsDlgClosed()
{
  _enableActions(true);
}

void
MmomeGui::_generateDebugPlots()
{
	_dbgPlotProc = new QProcess();
	connect(_dbgPlotProc, SIGNAL(started()), this, SLOT(on_debugPlotStarted()));
	connect(_dbgPlotProc, SIGNAL(error(QProcess::ProcessError)), this,
			SLOT(on_debugPlotError(QProcess::ProcessError)));
	connect(_dbgPlotProc, SIGNAL(finished(int, QProcess::ExitStatus)), this,
			SLOT(on_debugPlotFinished(int, QProcess::ExitStatus)));

	QString python_interpreter = "python";

	QString plotScript = _utils->appCommand(CMD_PLOT_DEBUG);
	QString path = _utils->appDir(MMOC_SOURCE) + SLASH + plotScript;

	QString modelName = Editor::instance()->activeBaseFileName();
	// TODO: avoid hardcoding this filename.
	QString stateLog = _utils->appDir(MMOC_OUTPUT) + SLASH + modelName + SLASH + "state_log";
	QString vars = _runDlg->_plotVars->text();

	QStringList args;
	args << path << stateLog << vars;

	_dbgPlotProc->start(python_interpreter, args);
}

void
MmomeGui::on_debugPlotStarted()
{
	_compiler_msg->append("\nGenerating debug plots for selected variables. Please wait... ");
}

void
MmomeGui::on_debugPlotError(QProcess::ProcessError error)
{
	QMessageBox messageBox;
	QString message = "Failed to generate debug plots. Make sure Python is installed in your system.";
	messageBox.critical(0, "Error", message);

	_dbgPlotProc->deleteLater();
}

void
MmomeGui::on_debugPlotFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
	if(exitCode != DBG_PLOT_SUCCESS)
	{
		QMessageBox messageBox;
		QString message = "Failed to generate debug plots. ";

		switch(exitCode)
		{
		case DBG_PLOT_ERR_MATPLOTLIB:
			message += "Make sure matplotlib is installed in your system.";
			break;
		case DBG_PLOT_ERR_PARSING:
			message += "There was an error while parsing the state log.";
			break;
		case DBG_PLOT_ERR_UNKNOWN_VAR:
			message += "One of the specified variables could not be found. Make sure it appears in the output list as well.";
			break;
		case DBG_PLOT_ERR_PLOTTING:
			message += "There was an error while rendering the plots.";
			break;
		case DBG_PLOT_ERR_DEFAULT:
		default:
		...
		_compiler_msg->moveCursor(QTextCursor::End);
		_compiler_msg->insertPlainText("failed!");
		_compiler_msg->moveCursor(QTextCursor::End);
	}
	else
	{
		_compiler_msg->moveCursor(QTextCursor::End);
		_compiler_msg->insertPlainText("done!");
		_compiler_msg->moveCursor(QTextCursor::End);
	}
...
MmomeGui::_run(QString name)
{
  ...
  env.insert("MMOC_ENGINE", QDir(_utils->appDir(MMOC_ENGINE)).absolutePath()); // Add an environment variable
  env.insert("MMOC_LIBS", QDir(_utils->appDir(MMOC_LIBS)).absolutePath()); // Add an environment variable
  env.insert("MMOC_MODELS", QDir(_utils->appDir(MMOC_MODELS)).absolutePath()); // Add an environment variable
  env.insert("MMOC_SRC", QDir(_utils->appDir(MMOC_SOURCE)).absolutePath()); // Add an environment variable
  env.insert("MMOC_PACKAGES",
      QDir(_utils->appDir(MMOC_PACKAGES)).absolutePath()); // Add an environment variable
  env.insert("MMOC_LIBRARIES",
      QDir(_utils->appDir(MMOC_LIBRARIES)).absolutePath()); // Add an environment variable
  ...
  env.insert("LD_LIBRARY_PATH",
      QDir(_utils->appDir(MMOC_LIBRARIES)).absolutePath() + "/../lib"); // Add an environment variable
  _proc->setProcessEnvironment(env);
  _proc->start(_utils->appDir(MMOC_BIN) + SLASH + sim, args);
}

void
MmomeGui::_simulation_message()
{
...
{
  _settings_only = true;
  on_actionCompileAndRun_triggered ();
}

void
MmomeGui::on_actionCompileAndRun_triggered()
{
  if(Editor::instance()->activeBaseFileName().isEmpty())
  {
    return;
...
bool
MmomeGui::_plotScript()
{
  QString name = Editor::instance()->activeBaseFileName();
  QFile file(
      _utils->appDir(MMOC_OUTPUT) + SLASH + name + SLASH + name
          + QString(".plt"));
  bool ret = false;
  if(!file.open(QIODevice::ReadWrite | QIODevice::Truncate))
  {
    QMessageBox::critical(this, QString(tr("Error")),
        QString(tr("Can't open file ")) + name + QString(".plt"));
    file.close();
    return false;
  }
  QString _data;
  ...
  for(int k = 0; k < _model->rowCount(); k++)
  {
    ...
    for(int g = 0; g < it->rowCount(); g++)
    {
      ...
      if(c->checkState() == Qt::Checked)
      {
        ...
        SLASH).append(_model->item(k)->text()).append(SLASH).append(c->text()).append(
            ".dat\"");
        c = it->child(g, 2);
        if(c->text() != "None")
        {
  ...
  {
    QMessageBox::critical(this, QString(tr("Error")),
        QString(tr("Can't write data ")) + name + QString(".plt"));
    file.close();
    return false;
  }
  if(!file.flush())
  {
    QMessageBox::critical(this, QString(tr("Error")),
        QString(tr("Can't save file ")) + name + QString(".plt"));
    file.close();
    return false;
  }
  return ret;
...
void
MmomeGui::cleanBuildDir(int idx)
{
  if(_settings_only)
    return;
  QDir buildDir(_utils->appDir(MMOC_BUILD));
  ...
  QFileInfo makefi(
      buildDir.absolutePath() + SLASH + Editor::instance()->baseFileName(idx)
          + QString(".makefile"));
  QFileInfo cfi(
      buildDir.absolutePath() + SLASH + Editor::instance()->baseFileName(idx)
          + QString(".c"));
...
MmomeGui::_compile(bool dbg)
{
  ...
  buildDir.cd(Editor::instance()->activeBaseFileName());
  QString fileName(
      buildDir.absolutePath() + SLASH + Editor::instance()->activeBaseFileName()
          + QString(".part"));
  ...
  QFile::copy(
      modelFile.absoluteDir().absolutePath() + SLASH
          + Editor::instance()->activeBaseFileName() + QString(".part"),
      fileName);
  _compiler_msg->setPlainText(
  ...
  if(dbg)
  {
    ...
    if(_runDlg->shouldGenerateDebugPlots())
      flags.append("-d SD_DBG_StateLog ");
    flags.append(" ");
    set += flags;
  }
  set += "-o " + buildDir.absolutePath() + SLASH
      + Editor::instance()->activeBaseFileName();
  args << set;
...
MmomeGui::_make_finished(int exitCode, QProcess::ExitStatus exitStatus)
{
  if(exitStatus == QProcess::NormalExit)
  {
    QString execName = _utils->appDir(MMOC_BUILD) + SLASH
        + Editor::instance()->activeBaseFileName() + QString(SLASH)
        + Editor::instance()->activeBaseFileName();
#ifdef _WIN32
    execName += ".exe";
#endif
  ...
  _compiler_msg->moveCursor(QTextCursor::End);
  _compiler_msg->ensureCursorVisible();
  _run(Editor::instance()->activeBaseFileName());
}
...
MmomeGui::_comp_finished(int exitCode, QProcess::ExitStatus exitStatus)
{
  if(exitStatus == QProcess::NormalExit)
  {
    ...
    QFileInfo cfi(
        buildDir.absolutePath() + SLASH
            + Editor::instance()->activeBaseFileName() + QString(".c"));
    _compiler_msg->setPlainText(
        _compiler_msg->toPlainText() + QString("\n")
            + _proc->readAllStandardError());
    QByteArray logs = _proc->readAllStandardOutput();
    ...
    QFileInfoList filst = modeldir.entryInfoList(
        QStringList() << "*.c" << "*.h");
    ...
    QFileInfo makefi(
        buildDir.absolutePath() + SLASH
            + Editor::instance()->activeBaseFileName() + QString(".makefile"));
    if(!makefi.exists())
    {
      _compiler_msg->setPlainText(
    ...
    _compiler_msg->setPlainText(
        _compiler_msg->toPlainText() + QString("\n") + QString("make -f ")
            + buildDir.absolutePath() + SLASH + makefi.fileName());
...
void
MmomeGui::on_actionAbort_triggered()
{
  if(Editor::instance()->activeBaseFileName().isEmpty())
  {
    return;
...
MmomeGui::on_actionClear_Log_triggered()
{
  ...
  outputDir.mkdir(Editor::instance()->activeBaseFileName());
  outputDir.cd(Editor::instance()->activeBaseFileName());
  QStringList dirs = outputDir.entryList(
      QStringList() << "*.log" << "*.dat" << "*.plt");
...
void
MmomeGui::_deleteVariables(QString name)
{
  int idx = -1;
  for(int k = 0; k < _model->rowCount(); k++)
  {
...
void
MmomeGui::_openRecentFiles()
{
  QAction *action = qobject_cast<QAction *>(sender());
  if(action)
  {
    _loadFile(action->data().toString());
  }
}

void
MmomeGui::_updateRecentFileActions()
{
...
void
MmomeGui::_loadFile(QString fileName)
{
  _editModel(fileName);
  _addVariables();
...
void
MmomeGui::_setCurrentFile(const QString fileName)
{
  _curFile = fileName;
...
void
MmomeGui::_plot_finished(int exitCode, QProcess::ExitStatus exitStatus)
{
...
QString
MmomeGui::_strippedName(const QString &fullFileName)
{
  return QFileInfo(fullFileName).fileName();
}

void
MmomeGui::_log_finished(int exitCode, QProcess::ExitStatus exitStatus)
{
  QDir outputDir(_utils->appDir(MMOC_OUTPUT));
  outputDir.cd(Editor::instance()->activeBaseFileName());
  QFileInfo logfi(
      outputDir.absolutePath() + SLASH
          + Editor::instance()->activeBaseFileName()
          + QString(".log"));
  if(logfi.size())
  {
    _editModel(logfi.absoluteFilePath());
  }
