...
void
RunDlg::on__debugChk_stateChanged (int state)
{
	bool enabled = state == Qt::Checked && _selectedSolverSupportsDebugPlots();
	_plotVarsChk->setEnabled(enabled);
	if(enabled && _plotVarsChk->checkState() == Qt::Checked)
		_plotVars->setEnabled(true);
	else
		_plotVars->setEnabled(false);
}

void
RunDlg::on__plotVarsChk_stateChanged (int state)
{
	bool enabled = state == Qt::Checked;
	_plotVars->setEnabled(enabled);
}
...
void
RunDlg::on__solver_currentIndexChanged(int index)
{
	bool dbgPlotsSupported = _selectedSolverSupportsDebugPlots();
	bool shouldEnablePlotVarsChk = dbgPlotsSupported && _debugChk->isChecked();
	bool shouldEnablePlotVars = shouldEnablePlotVarsChk &&
								_plotVarsChk->isChecked();
	_plotVarsChk->setEnabled(shouldEnablePlotVarsChk);
	_plotVars->setEnabled(shouldEnablePlotVars);
}

bool
RunDlg::_selectedSolverSupportsDebugPlots()
{
	QString selectedSolver = solver();
	// Debug plots only available for QSS methods.
	return
		selectedSolver != "DASSL"  &&
		selectedSolver != "DOPRI";
}
...
bool
RunDlg::shouldGenerateDebugPlots ()
{
  return enableDebug()
		  && _plotVarsChk->isEnabled()
		  && _plotVarsChk->isChecked()
		  && !_plotVars->text().isEmpty();
}
